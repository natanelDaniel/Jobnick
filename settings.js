...
class PopupManager {
    ...
    async saveAIAgentData() {
        const aiAgentData = {
            ...
            aiSubmissionMode: document.getElementById('aiSubmissionMode')?.checked === false, // Default to false (no send)
        ...
        try {
            ...
            this.logDebug(`❌ Error saving AI Agent settings: ${error.message}`, 'error');
            this.showNotification('Failed to save AI Agent settings', 'error');
        }
    }

    async startAISearch() {
        try {
            this.logDebug('🚀 Initializing AI Job Search...', 'info');
            ...
            if (!currentApiKey) {
                try {
                    ...
                    currentApiKey = sync?.geminiApiKey?.trim() 
                    ...
                    this.logDebug(`Error loading API key from storage: ${e.message}`, 'warning');
                }
            }

            if (!currentApiKey) {
                const errorMsg = 'Gemini API key is required. Please enter your API key in the field above.';
                this.logDebug(errorMsg, 'error');
                this.showNotification(errorMsg, 'error');
                return;
            }

            this.logDebug(`✅ API key found: ${currentApiKey.substring(0, 10)}...`, 'success');

            // Update the field if it was loaded from storage
            if (document.getElementById('geminiApiKey').value.trim() === '') {
                document.getElementById('geminiApiKey').value = currentApiKey;
            }

            // Test background script connection first
            this.logDebug('🔧 Testing background script connection...', 'info');
            try {
                const pingResponse = await chrome.runtime.sendMessage({ action: 'ping' });
                this.logDebug('✅ Background script connection OK', 'success');
            } catch (e) {
                const errorMsg = `Background script not responding: ${e.message}`;
                this.logDebug(errorMsg, 'error');
                this.showNotification(errorMsg, 'error');
                return;
            }

            // Store the API key in storage for persistence
            try {
                await chrome.storage.sync.set({ geminiApiKey: currentApiKey });
                this.logDebug('✅ API key saved to storage', 'success');
            } catch (e) {
                this.logDebug(`Warning: Could not save API key to storage: ${e.message}`, 'warning');
            }
            ...
            this.logDebug(`📊 AI Settings: ${JSON.stringify(settings)}`, 'info');

            // Ensure the background agent has the key
            this.logDebug('🔑 Setting API key in background script...', 'info');
            ...
            if (!setKeyResponse?.success) {
                const errorMsg = 'Failed to set API key in background script';
                this.logDebug(errorMsg, 'error');
                this.showNotification(errorMsg, 'error');
                return;
            }
            
            this.logDebug('✅ API key set in background script', 'success');

            // Start the AI job search
            this.logDebug('🚀 Launching AI job search...', 'info');
            ...
            if (!startResponse?.success) {
                const errorMsg = `Failed to start AI job search: ${startResponse?.error || 'Unknown error'}`;
                this.logDebug(errorMsg, 'error');
                this.showNotification(errorMsg, 'error');
                return;
            }
            ...
            this.logDebug(`❌ Critical error starting AI search: ${error.message}`, 'error');
            this.logDebug(`Error stack: ${error.stack}`, 'error');
            this.incrementErrorCount();
            this.showNotification(`Error starting AI search: ${error.message}`, 'error');
    ...
    populateAIAgentForm(aiAgent) {
        ...
        if (aiSubmissionModeField) {
            ...
            aiSubmissionModeField.checked = aiAgent.aiSubmissionMode !== true  ;
...
async testGeminiAPI() {
  try {
    ...
    async renderHistoryTable() {
        try {
            ...
            applicationHistory.forEach(item => {
                ...
                // Get job title from different possible fields
                const jobTitle = item.jobTitle || item.title || 'Unknown Job';
                
                // Get company from different possible fields
                const company = item.company || 'Unknown Company';
                
                // Get location from different possible fields
                ...
                if (statusText.includes('Test Mode')) {
                    ...
                    statusText = `❌ ${statusText}`;
                } else if (statusText.includes('Filled')) {
                    statusClass = 'status-info';
                    statusText = `📝 ${statusText}`;
                }
                
                // Add method information to status if available
                if (item.method) {
                    statusText += ` (${item.method})`;
                }
    ...
    setupEventListeners() {
        ...
        document.getElementById('aiAgentForm')?.addEventListener('submit', (e) => {
            e.preventDefault();
            this.saveAIAgentData();
        });

        // Toggle switch for submission mode
        document.getElementById('aiSubmissionMode')?.addEventListener('change', (e) => {
            // Update both status and main text immediately (don't wait for save)
            this.updateToggleStatus(e.target.checked);
            this.updateToggleMainText(e.target.checked);
            
            // Auto-save the changes immediately
            this.saveAIAgentData().catch(error => {
                console.warn('Auto-save failed:', error);
                // If auto-save fails, revert the toggle
        ...
        // Toggle switch for Hebrew messages
        document.getElementById('hebrewMessages')?.addEventListener('change', (e) => {
            // Update both status and main text immediately (don't wait for save)
            this.updateHebrewToggleStatus(e.target.checked);
            this.updateHebrewToggleMainText(e.target.checked);
        ...
        // AI controls
        document.getElementById('startAISearch')?.addEventListener('click', () => this.startAISearch());
        document.getElementById('stopAISearch')?.addEventListener('click', () => this.stopAISearch());
        ...
        // Quick action buttons
        document.getElementById('quickUploadResume')?.addEventListener('click', () => this.quickUploadResume());
        document.getElementById('quickTestUpload')?.addEventListener('click', () => this.quickTestUpload());

        // Beta Zone toggle
        document.getElementById('betaZoneToggle')?.addEventListener('click', () => this.toggleBetaZone());
    }
    ...
    toggleBetaZone() {
        const toggleBtn = document.getElementById('betaZoneToggle');
        const content = document.getElementById('betaZoneContent');
